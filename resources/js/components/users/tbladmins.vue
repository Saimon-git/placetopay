<template>
    <div class="w-full">
        <table class="table-auto w-full mt-2 text-left">
            ...
            <tbody >
                <tr class="border-b" v-for="admin in admins" :key="admin.id">
                    <td class="px-4 py-2">
                        <img :src="admin.avatar" alt="Avatar" class="rounded-full user-avatar-image items-center justify-center">
                    ...
                    <td class="px-4 py-2">
                        <dropdown>
                            ...
                            <template slot="list">
                                <li class="">
                                    <a @click="show(admin,'dlgResetPasswordAdmin')" class="hover:bg-green hover:text-white rounded-t py-2 px-4 block whitespace-no-wrap" href="#">
                                        Reset Password
                                    </a>
                                </li>
                                <li class="">
                                    <a @click="show(admin,'dlgDeleteAdmin')" class="hover:bg-green hover:text-white rounded-b py-2 px-4 block whitespace-no-wrap" href="#">
                                        Delete Member
                                    </a>
                                </li>
                            </template>
        ...
        <modal-delete id="dlgDeleteAdmin" title="Are you sure you want to remove the membership?">
...
<script>
    ...
    import DELETE_ADMIN_ROLE from "../../graphql/users/delete_admin_role.graphql";
    ...
    export default {
        ...
        methods: {
            ...
            async destroy(){
                ...
                .then(() => {
                    Vue.$toast.clear();
                    Vue.$toast.success('The Users has been Deleted');

                    this.toggleModal('dlgDeleteAdmin');
                    this.reload(1);
                });
            },
            ...
            async removeAdminRole(){
                Vue.$toast.info('Adding..');

                await this.$apollo.mutate({
                    mutation: DELETE_ADMIN_ROLE,
                    variables: {
                        id: this.user.id,
                    },
                })
                .then(() => {
                    Vue.$toast.clear();
                    Vue.$toast.success('The User has been Removed as Admin');

                    this.toggleModal('dlgRemoveAdminRole');
                    this.reload(1);
                });
            },

            async reload(page) {
                let query = {page_admins: page};
